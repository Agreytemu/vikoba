...
/**
 * Terminal failure screen. The status comes straight from the backend payment
 * status, so a cancelled request reads differently from a declined one.
 */
const DepositFailed: FC<DepositFailedProps> = ({
  amount,
  status = "FAILED",
  reference,
  onRetry,
  ...
  <div className="rounded-2xl border border-slate-200 bg-white p-8 text-center shadow-card dark:border-slate-800 dark:bg-slate-900">
    ...
    <h1 className="mt-4 font-display text-2xl font-semibold">
      {STATUS_TITLE[status] ?? "Deposit failed"}
    </h1>
    <p className="mx-auto mt-2 max-w-sm text-sm text-slate-500 dark:text-slate-400">
      Your deposit could not be completed. Nothing was charged to your mobile
      money.
    </p>
