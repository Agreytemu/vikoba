...
import { MIN_PAYMENT_TZS } from "@/lib/payments";
import { formatDepositMoney, MAX_DEPOSIT_TZS } from "./depositFlow";
...
/** TZS amount field with inline validation and a min/max hint. */
const DepositAmountInput: FC<DepositAmountInputProps> = ({
  id = "deposit-amount",
  value,
  error,
  onChange,
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-card dark:border-slate-800 dark:bg-slate-900">
      <div className="flex items-center justify-between gap-3">
        ...
        <span className="text-[11px] text-slate-400">
          Min {formatDepositMoney(MIN_PAYMENT_TZS)} · Max{" "}
          {formatDepositMoney(MAX_DEPOSIT_TZS)}
        </span>
      </div>
      ...
      {error ? (
        ...
        <p id={hintId} className="mt-2 text-[11px] text-slate-400">
          Enter the amount you want to move from your mobile money into your savings.
        </p>
      )}
    </section>
  );
