...
const labels: Record<LoanStatus, string> = { draft: "Draft", submitted: "Submitted", under_review: "Under review", approved: "Approved", rejected: "Rejected", cancelled: "Cancelled", disbursed: "Disbursed" };
...
const LoansView = () => {
  ...
  const runAction = async (actionName: "submit" | "review" | "approve" | "reject" | "disburse" | "repay", payload: Record<string, string> = {}) => {
    try {
      ...
      setModal(null); setNotes(""); setInstallmentNumber(""); setRepayAmount(""); setApprovedAmount("");
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      <div className="grid gap-5 lg:grid-cols-3"><section className="rounded-lg border p-4 lg:col-span-2"><h2 className="font-medium">Loan details</h2><dl className="mt-3 grid grid-cols-2 gap-3 text-sm"><div><dt className="text-slate-500">Requested amount</dt><dd>{money(loan.requested_amount)}</dd></div><div><dt className="text-slate-500">Repayment period</dt><dd>{loan.repayment_period_months} months</dd></div><div><dt className="text-slate-500">Purpose</dt><dd>{loan.purpose}</dd></div><div><dt className="text-slate-500">Security</dt><dd>{loan.security_type.replace(/_/g, " ")}</dd></div>{loan.collateral_description && <div className="col-span-2"><dt className="text-slate-500">Collateral</dt><dd>{loan.collateral_description}</dd></div>}<div><dt className="text-slate-500">Employer</dt><dd>{loan.employer || "—"}</dd></div><div><dt className="text-slate-500">Payroll number</dt><dd>{loan.payroll_number || "—"}</dd></div>{loan.approved_amount != null && <div><dt className="text-slate-500">Approved amount</dt><dd className="font-medium">{money(loan.approved_amount)}</dd></div>}</dl></section><section className="rounded-lg border p-4"><h2 className="font-medium">Member summary</h2><dl className="mt-3 space-y-2 text-sm"><div>Member: {loan.member_summary.membership_number}</div><div>Joined: {loan.member_summary.membership_date}</div><div>Deposits: {money(loan.member_summary.deposits)}</div><div>Existing loans: {loan.member_summary.existing_loans}</div><div>Outstanding: {money(loan.member_summary.outstanding_balance)}</div></dl></section></div>
      ...
      {loan.loan_account && <section className="rounded-lg border p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-medium">Repayment schedule</h2><p className="mt-1 text-sm text-slate-500">{loan.loan_account.interest_type === "flat" ? "Flat" : "Reducing-balance"} interest at {loan.loan_account.interest_rate}%.</p></div><div className="text-right text-sm"><div>Amount repayable: <strong>{money(loan.loan_account.total_repayable)}</strong></div><div>Outstanding: <strong>{money(loan.loan_account.outstanding_balance)}</strong></div>{Number(loan.loan_account.outstanding_penalty) > 0 && <div className="text-red-600 dark:text-red-400">incl. penalty {money(loan.loan_account.outstanding_penalty)}</div>}</div></div><div className="mt-4 overflow-x-auto rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-950"><table className="w-full min-w-[700px] text-left text-sm"><thead className="border-b border-slate-200 bg-slate-100/80 text-xs font-semibold uppercase tracking-wide text-slate-600 dark:border-slate-800 dark:bg-slate-900/80 dark:text-slate-300"><tr><th className="px-4 py-3">Installment</th><th className="px-4 py-3">Due date</th><th className="px-4 py-3">Principal</th><th className="px-4 py-3">Interest</th><th className="px-4 py-3">Amount due</th><th className="px-4 py-3">Status</th></tr></thead><tbody>{loan.loan_account.schedule.map((item: LoanInstallment) => { return <tr key={item.installment_number} className="border-b border-slate-200/90 text-slate-700 transition-colors hover:bg-blue-50/60 last:border-0 dark:border-slate-800/90 dark:text-slate-200 dark:hover:bg-blue-950/30"><td className="px-4 py-3">{item.installment_number}</td><td className="px-4 py-3">{new Date(`${item.due_date}T00:00:00`).toLocaleDateString()}</td><td className="px-4 py-3">{money(item.principal_due)}</td><td className="px-4 py-3">{money(item.interest_due)}</td><td className="px-4 py-3 font-medium">{money(item.outstanding_due)}{Number(item.partially_paid_amount) > 0 && <span className="block text-xs font-normal text-slate-500">of {money(item.total_due)}</span>}</td><td className="px-4 py-3"><Badge variant={item.is_paid ? "default" : item.status === "OVERDUE" ? "destructive" : "secondary"}>{item.is_paid ? `Paid${item.paid_at ? ` · ${new Date(item.paid_at).toLocaleDateString()}` : ""}` : item.status === "PARTIALLY_PAID" ? "Partially paid" : item.status.replace(/_/g, " ")}</Badge></td></tr>; })}</tbody></table></div></section>}
      ...
      <Modal isOpen={modal !== null} onClose={() => setModal(null)} title={modal === "approve" ? "Approve application" : modal === "reject" ? "Reject application" : modal === "repay" ? "Post installment repayment" : "Disburse loan"}><div className="space-y-4">{(modal === "disburse" || modal === "repay") && <select className="h-10 w-full rounded border px-3 text-slate-900" value={accountNumber} onChange={(event) => setAccountNumber(event.target.value)}><option value="">Select member account</option>{accounts.filter((account) => account.is_active).map((account) => <option key={account.account_number} value={account.account_number}>{account.account_number} — balance {money(account.balance)}</option>)}</select>}{modal === "repay" && <select className="h-10 w-full rounded border px-3 text-slate-900" value={installmentNumber} onChange={(event) => { setInstallmentNumber(event.target.value); const selected = loan.loan_account?.schedule.find((item) => item.installment_number === Number(event.target.value)); setRepayAmount(selected ? String(selected.outstanding_due) : ""); }}><option value="">Select unpaid installment</option>{loan.loan_account?.schedule.filter((item) => !item.is_paid).map((item) => <option key={item.installment_number} value={item.installment_number}>Installment {item.installment_number} — due {item.due_date} — {money(item.outstanding_due)}{Number(item.partially_paid_amount) > 0 ? ` of ${money(item.total_due)}` : ""}</option>)}</select>}{modal === "repay" && <div><label className="mb-1 block text-sm">Amount to repay</label><Input type="number" min="1" value={repayAmount} onChange={(event) => setRepayAmount(event.target.value)} placeholder="Amount due" /></div>}{modal === "approve" && <div><label className="mb-1 block text-sm">Approved amount (optional — defaults to requested)</label><Input type="number" min="1" value={approvedAmount} onChange={(event) => setApprovedAmount(event.target.value)} placeholder={`${money(loan.requested_amount)}`} /></div>}<textarea className="min-h-24 w-full rounded border p-3 text-slate-900" value={notes} onChange={(event) => setNotes(event.target.value)} placeholder={modal === "reject" ? "Rejection reason (required)" : modal === "repay" ? "Repayment narration (optional)" : "Notes (optional)"} /><Button disabled={action.isPending || (modal === "reject" && !notes.trim()) || ((modal === "disburse" || modal === "repay") && !accountNumber) || (modal === "repay" && (!installmentNumber || !repayAmount))} variant={modal === "reject" ? "destructive" : "default"} onClick={() => modal === "approve" ? runAction("approve", { approval_notes: notes, approved_amount: approvedAmount || "" }) : modal === "reject" ? runAction("reject", { rejection_reason: notes }) : modal === "repay" ? runAction("repay", { account_number: accountNumber, installment_number: installmentNumber, amount: repayAmount, narration: notes }) : runAction("disburse", { account_number: accountNumber, disbursement_notes: notes })}>{modal === "approve" ? "Confirm approval" : modal === "reject" ? "Confirm rejection" : modal === "repay" ? "Confirm repayment" : "Confirm disbursement"}</Button></div></Modal>
    </div>
  );
};
