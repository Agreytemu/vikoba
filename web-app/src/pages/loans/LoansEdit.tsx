...
const LoansEdit = () => {
  ...
  // When staff start a new application from a member page the member is
  // pre-selected via ?member= so they are not searched for twice.
  const [searchParams] = useSearchParams();
  const prefillMember = searchParams.get("member") || "";
  ...
  const form = useForm<FormValues>({
    defaultValues: { ...defaultValues, member: prefillMember },
  });
          ...
          <section className="grid gap-4 md:grid-cols-2">
            ...
            {selectedType && (
              <p className="rounded bg-white/70 p-3 text-sm text-slate-600 md:col-span-2">
                Amount: {formatMoney(Number(selectedType.min_amount))}–
                {formatMoney(Number(selectedType.max_amount))} · Maximum
                  ...
                  <div className="space-y-2 text-sm">
                    {application?.guarantors.length ? application.guarantors.map((guarantor) => (
                      <div key={guarantor.id} className="flex items-center justify-between gap-3 rounded border px-3 py-2">
                        <span>{guarantor.member_name} · {formatMoney(Number(guarantor.guaranteed_amount))}</span>
