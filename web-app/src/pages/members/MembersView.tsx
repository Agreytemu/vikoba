                    ...
                    {memberAccounts?.length ? (
                      memberAccounts?.slice(0, 10).map((account) => (
                        <TableRow key={account.account_number}>
                          ...
                          <TableCell>{formatMoney(Number(account.balance))}</TableCell>
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-950">
            <Card className="border-0 bg-transparent shadow-none dark:border-0 dark:bg-transparent xl:col-span-2" x-chunk="dashboard-01-chunk-4">
              <CardHeader className="flex flex-row items-center justify-between gap-3">
                ...
                {canViewLoans && <Link className="text-sm text-blue-700 underline dark:text-blue-400" to={`/loans/edit?member=${member?.membership_number || ""}`}>New application</Link>}
              </CardHeader>
              <CardContent>
                  ...
                  <TableBody>
                    {memberLoans.length ? memberLoans.map((loan) => (
                      <TableRow key={loan.application_number}>
                        ...
                        <TableCell>{formatMoney(Number(loan.requested_amount))}</TableCell>
