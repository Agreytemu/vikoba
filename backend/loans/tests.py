...
class MemberLoanSelfServiceTest(TestCase):
	"""The member self-service loan flow (/loans/me)."""

	def setUp(self):
		self.client = APIClient()
		user = User.objects.create_user(
			email="selfloan@example.com",
			username="selfloan",
			password="test-password-123",
			role=User.MEMBER,
			email_verified=True,
	...
	def test_member_can_create_and_view_own_draft(self):
		response = self.client.post(
			"/api/v1/loans/me/",
			{
				"loan_type": self.loan_type.id,
		...
		detail = self.client.get(f"/api/v1/loans/me/{application_number}/")
		self.assertEqual(detail.status_code, 200)
		self.assertEqual(detail.data["member"], self.member.membership_number)

	def test_unverified_member_cannot_submit_loan(self):
		response = self.client.post(
			"/api/v1/loans/me/",
			{
				"loan_type": self.loan_type.id,
		...
		submit = self.client.post(f"/api/v1/loans/me/{application_number}/submit/")
		self.assertEqual(submit.status_code, 403)
		self.assertTrue(submit.data.get("verification_required"))

	def test_verified_member_reaches_eligibility_calculator(self):
		response = self.client.get("/api/v1/loans/me/eligibility/")
		self.assertEqual(response.status_code, 200)
		self.assertIn("products", response.data)
		self.assertIn("current_deposits", response.data)
