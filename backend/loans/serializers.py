...
from members.validators import validate_uploaded_file
...
class MemberLoanApplicationListSerializer(serializers.ModelSerializer):
    """Compact view of a member's own loan applications (self-service)."""
    loan_type_name = serializers.CharField(source="loan_type.name", read_only=True)
...
class MemberLoanApplicationCreateSerializer(serializers.ModelSerializer):
    """Member self-service application creation (always starts as DRAFT)."""
...
class MemberRepaySerializer(serializers.Serializer):
    ...
    def validate_installment_number(self, value):
        ...
        if not schedule.filter(installment_number=value).exists():
            raise serializers.ValidationError("That installment does not exist on this loan.")
        if schedule.get(installment_number=value).is_paid:
            raise serializers.ValidationError("That installment has already been paid.")
        return value
