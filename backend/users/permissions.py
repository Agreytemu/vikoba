...
class HasCommunityAccess(RolePermission):
    """Staff who may publish announcements and manage meetings."""

    allowed_roles = ALL_BUSINESS_ROLES


class IsMember(BasePermission):
    """Authenticated member (role ME) with a linked Member profile."""

    message = "Only members can use this endpoint."
    ...
    def has_permission(self, request, view):
        ...
        return bool(
            user
            and user.is_authenticated
            and (user.is_superuser or user.role == User.MEMBER)
            and getattr(user, "member", None) is not None
        )
