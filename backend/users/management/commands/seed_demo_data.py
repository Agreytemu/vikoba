...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        # Never seed demo users/records (including an admin with a well-known
        # password) against a non-dev database. SQLite is only used locally; a
        # DATABASE_URL (Postgres via a provider) also counts as non-local even
        # if DATABASE_MODE was left at its default.
        if not settings.DEBUG and (
            settings.DATABASE_MODE != "sqlite" or getattr(settings, "DATABASE_URL", None)
        ):
            raise CommandError(
                "seed_demo_data is disabled outside local development "
                "(DEBUG=False). It creates well-known demo records and an "
                "admin superuser."
            )
        ...
        if not Member.objects.exists():
            ...
            EmploymentDetail.objects.create(
                ...
                employer_name="Vikoba Kidigitali",
